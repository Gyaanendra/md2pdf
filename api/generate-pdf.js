...
function renderKaTeX(md) {
  ...
  result = result.replace(/\$\$([\s\S]*?)\$\$/g, (_, tex) => {
    try {
      return `\n\n<div class="math-display">${katex.renderToString(tex.trim(), { displayMode: true, throwOnError: false, output: 'html', strict: false })}</div>\n\n`;
  ...
  result = result.replace(/(?<!\$)\$(?!\$)([^\$\n]+?)\$(?!\$)/g, (_, tex) => {
    try {
      return katex.renderToString(tex.trim(), { displayMode: false, throwOnError: false, output: 'html', strict: false });
    } catch { return `<span class="katex-error">${tex}</span>`; }
  });
    ...
    const pdf = await mdToPdf(
      { content: withMath },
      {
        ...
        pdf_options: {
          ...
          headerTemplate: `
            <div style="width:100%;padding:0 18mm;font-size:8px;color:#999;font-family:'Inter','Helvetica Neue',Arial,sans-serif;display:flex;justify-content:space-between;border-bottom:0.5px solid #e0e0e0;padding-bottom:4pt;">
              <span>${(title || 'Document').replace(/</g, '&lt;')}</span>
              <span>${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</span>
            </div>`,
          footerTemplate: `
            <div style="width:100%;padding:0 18mm;font-size:8px;color:#999;font-family:'Inter','Helvetica Neue',Arial,sans-serif;text-align:center;border-top:0.5px solid #e0e0e0;padding-top:4pt;">
